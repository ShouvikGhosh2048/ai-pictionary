...
export const newRound = action({
    ...
    handler: async (ctx, args) => {
      ...
      try {
        ...
        const imageDescription = `We are playing a pictionary game. Create an image for the topic ${answer}.
        ...
        const image = imageResponseParts[0]?.inlineData?.data;
        if (!image) { throw new Error("Didn't get an image"); }
